...
<body>
    ...
    (function() {
        ...
        function getSearchSnippet(articleId, query) {
            if (!searchData || !searchData[articleId] || !query) return '';
            ...
            var snippetRadius = 75;
            var start = Math.max(0, idx - snippetRadius);
            var end = Math.min(text.length, idx + query.length + snippetRadius);
            ...
            if (end < text.length) {
                ...
                if (spaceIdx2 > idx + query.length) end = spaceIdx2;
            }
        ...
        /* ============================================================
           ...
           ============================================================ */
        function highlightTermInBody(query) {
            highlightMatches = [];
        ...
        function fetchArticleContent(articleId) {
            ...
            xhr.onreadystatechange = function() {
                ...
                if (xhr.status === 200) {
                    ...
                    if (panelSearchQuery) {
                        var matchCount = highlightTermInBody(panelSearchQuery);
        ...
        function executePanelSearch() {
            ...
            var query = panelSearchQuery.toLowerCase();
            var results = [];
            ...
            for (var i = 0; i < catalog.articles.length; i++) {
                ...
                if ((a.title || '').toLowerCase().indexOf(query) !== -1) {
                    score = 3;
                }

                // Check author
                if (!score && (a.author || '').toLowerCase().indexOf(query) !== -1) {
                    score = 2;
                }

                // Check metadata (principles, applications, keywords)
                if (!score) {
                    var meta = (a.principles || []).join(' ') + ' ' +
                        (a.applications || []).join(' ') + ' ' +
                        (a.keywords || []).join(' ');
                    if (meta.toLowerCase().indexOf(query) !== -1) {
                        score = 2;
                    }
                }

                // Check full text body
                if (!score && searchData && searchData[a.id]) {
                    if (searchData[a.id].toLowerCase().indexOf(query) !== -1) {
                        score = 1;
                    }
                }

                if (score > 0) {
                    results.push({ article: a, score: score });
                }
            }
            ...
            renderPanelResults(panelFilteredArticles, panelSearchQuery);
        }

        function renderPanelResults(articles, query) {
            ...
            for (var i = 0; i < limit; i++) {
                ...
                var snippet = getSearchSnippet(a.id, query);
